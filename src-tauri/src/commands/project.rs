...
pub fn get_project_node_count(db: State<Database>, project_id: String) -> Result<i64, String> {
    db.get_project_node_count(&project_id).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn update_project_config(db: State<Database>, id: String, config: Option<String>) -> Result<(), String> {
    db.update_project_config(&id, config.as_deref()).map_err(|e| e.to_string())
}
